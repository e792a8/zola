...
static mut CONFIG_VALUE: JsonValue = JsonValue::Null;
...
fn sass_get_config(args: ArgumentResult, _visitor: &mut Visitor) -> SassResult<Value> {
    ...
    // SAFETY: only accessed within `compile_sass`
    Ok(unsafe { map_value(&CONFIG_VALUE, span) })
}
...
pub fn compile_sass(base_path: &Path, output_path: &Path, config: &Config) -> Result<()> {
    ...
    // SAFETY: only accessed within `compile_sass`
    unsafe { CONFIG_VALUE = to_value(config).unwrap() };
